...
import static zipkin2.storage.cassandra.v1.SessionFactory.Default.buildCluster;
import static zipkin2.storage.cassandra.v1.SessionFactory.Default.parseContactPoints;
...
public class SessionFactoryTest {
  ...
  public void contactPoints_hostAndPort() {
    assertThat(
            parseContactPoints(CassandraStorage.newBuilder().contactPoints("1.1.1.1:9142").build()))
  ...
  public void connectPort_singleContactPoint() {
    assertThat(
            buildCluster(CassandraStorage.newBuilder().contactPoints("1.1.1.1:9142").build())
                .getConfiguration()
                .getProtocolOptions()
                .getPort())
        .isEqualTo(9142);
  ...
  public void authProvider_defaultsToNone() {
    assertThat(
            buildCluster(CassandraStorage.newBuilder().build())
                .getConfiguration()
                .getProtocolOptions()
                .getAuthProvider())
  ...
  public void loadBalancing_settingLocalDcIgnoresOtherDatacenters() {
    DCAwareRoundRobinPolicy policy =
        toDCAwareRoundRobinPolicy(CassandraStorage.newBuilder().localDc("bar").build());
  ...
  public void maxConnections_defaultsTo8() {
    PoolingOptions poolingOptions =
        buildCluster(CassandraStorage.newBuilder().build()).getConfiguration().getPoolingOptions();
